...
import { Container, Typography, Box, Paper, Table, TableBody, TableCell, TableContainer, 
  TableHead, TableRow, TablePagination, Button, TextField, Dialog, DialogActions, 
  DialogContent, DialogTitle } from '@mui/material';
import { useGetAllStoresQuery, useCreateStoreMutation, useUpdateStoreMutation, useDeleteStoreMutation } from '../../store/api/storeApi';
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={5} align="center">No stores found</TableCell>
              </TableRow>
            ) : (
              storesData?.data.map((store) => (
                <TableRow key={store.id}>
                  ...
                  <TableCell>{store.average_rating} ({store.rating_count} ratings)</TableCell>
                  <TableCell>
                    <Button size="small" onClick={() => openEditDialog(store)}>Edit</Button>
                    <Button size="small" color="error" onClick={() => openDeleteDialog(store)}>Delete</Button>
                  </TableCell>
                </TableRow>
              ))
            )}
      ...
      {selectedStore && (
        <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
          ...
          <DialogActions>
            ...
            <Button onClick={handleDeleteStore} color="error">Delete</Button>
          </DialogActions>
        </Dialog>
      )}
    </Container>
